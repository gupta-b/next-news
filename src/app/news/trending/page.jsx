...
export default function TrendingPage() {
  ...
  // Function to generate mock trending news data
  const generateMockNews = (start, count) => {
    ...
    return Array.from({ length: count }, (_, i) => {
      const id = start + i
      const daysAgo = Math.floor(Math.random() * 2) // Trending news is more recent
  ...
  const loadMoreArticles = () => {
    ...
    setTimeout(() => {
      ...
      // Stop after 3 pages for trending news
      if (page >= 3) {
        setHasMore(false)
      }
    }, 1000)
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={cn("col-span-1 lg:col-span-2 space-y-6", selectedArticle ? "hidden lg:block" : "block")}>
          ...
          {groupedArticles.today && groupedArticles.today.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-xl font-semibold">{t("todaysTrending")}</h2>
                <Separator className="flex-1" />
              </div>
              <div className="space-y-4">
                {groupedArticles.today.map((article) => (
                  <TrendingNewsCard key={article.id} article={article} onClick={() => setSelectedArticle(article)} />
                ))}
              </div>
            </div>
          )}
          ...
          {groupedArticles.yesterday && groupedArticles.yesterday.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-xl font-semibold">{t("yesterdaysTrending")}</h2>
                <Separator className="flex-1" />
              </div>
              <div className="space-y-4">
          ...
          {!hasMore && <p className="py-4 text-center text-sm text-muted-foreground">{t("noMoreTrending")}</p>}
        </div>
        ...
        >
          {selectedArticle ? (
            ...
            <div className="bg-muted rounded-lg p-8 text-center h-[70vh] flex items-center justify-center">
              <div className="space-y-4">
                <h3 className="text-xl font-medium">{t("selectTrending")}</h3>
...
function TrendingNewsCard({ article, onClick }) {
